...
| (4) 주요 기능의 구현-1 |**식재료 이미지 인식 기능**<br> 1. 개요<br>사용자가 업로드한 식재료 이미지를 인공지능을 활용하여 자동으로 인식하고 분류하는 시스템을 구축하는 것을 목표로 한다. 이를 위해 최신 객체 탐지 모델인 YOLOv8을 활용하여 다양한 식재료를 인식할 수 있도록 구현하였다.<br><br>2. 데이터셋 구축<br>2.1 데이터 수집<br>- 데이터 출처: 인터넷 크롤링<br>- 수집 개수: 총 2,000장 이상의 이미지<br>- 식재료 종류: 15가지 (*당근, 양파, 대파, 버섯, 콩나물, 양배추, 달걀, 우유, 치즈, 버터, 닭고기, 돼지고기, 소고기, 고등어, 갈치*)<br><br>2.2 데이터 라벨링<br>- 라벨링 툴 사용: Roboflow<br>- Bounding Box 기준: 식재료의 중심을 포함하도록 설정<br><br>2.3 데이터 증강(Augmentation)<br>- 랜덤 회전<br>- 밝기 조절<br>- 크롭 및 확대<br>- 노이즈 추가<br>- 총 4500장 이상의 이미지로 증강<br><br>2.4 Train / Validation / Test 데이터 분할<br>- Train: 4,149장<br>- Validation: 161장<br>- Test: 164장<br><br>3. 모델 학습<br>3.1 환경 설정<br>- 개발 환경: Google Colab Pro (Tesla T4 GPU 활용)<br>- 필요 라이브러리 설치:<br>```!pip install ultralytics```<br><br>3.2 YOLOv8 모델 설정<br>- YOLOv8 모델 다운로드 및 학습 설정<br><br>4. 모델 평가<br>- Precision (정밀도): 0.612<br>- Recall (재현율): 0.694<br>- mAP@50 (Mean Average Precision): 평균 0.688<br><br> 5. 모델 테스트 및 결과 분석<br>5.1 테스트 이미지 예측 실행 |
