...
| (4) 주요 기능의 구현 | 
# <첫번째 주요 기능 - 식재료 이미지 인식 기능>

## 1. 개요
사용자가 업로드한 식재료 이미지를 인공지능을 활용하여 자동으로 인식하고 분류하는 시스템을 구축하는 것을 목표로 한다. 이를 위해 최신 객체 탐지 모델인 **YOLOv8**을 활용하여 다양한 식재료를 인식할 수 있도록 구현하였다.
...
- **수집 개수:** 총 **4,500장 이상**  
- **식재료 종류 (15가지):**  
  - 당근, 양파, 대파, 버섯, 콩나물, 양배추, 달걀, 우유, 치즈, 버터, 닭고기, 돼지고기, 소고기, 고등어, 갈치  
...
### 2.3 데이터 라벨링  
- **라벨링 툴:** [Roboflow](https://roboflow.com/)  
- **Bounding Box 기준:** 식재료의 중심을 포함하도록 설정  
- **데이터 분할:**  
  - **Train:** 4,149장  
  - **Validation:** 161장  
  - **Test:** 164장  
...
### 3.1 환경 설정  
- **개발 환경:** Google Colab Pro (Tesla T4 GPU 활용)  
- **필요 라이브러리 설치:**  
  ```python
  !pip install ultralytics roboflow
  
### 3.2 YOLOv8 모델 설정
- **YOLOv8 모델 다운로드 및 학습 설정**
  ```python
...
### 3.3 모델 평가
- **Precision (정밀도):** 0.64 ~ 0.90
- **Recall (재현율):** 0.44 ~ 1.0
- **mAP@50 (Mean Average Precision):** 평균 0.77
